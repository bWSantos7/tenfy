...
export const InteractiveTutorial: React.FC<{ user: User | null }> = ({ user }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9998] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6">
      <div className="bg-bg-card border border-border-subtle rounded-t-2xl sm:rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
        {/* Cabeçalho */}
        ...
        {/* Conteúdo do passo */}
        <div className="px-6 pb-2 pt-3 flex flex-col items-center text-center">
          <div className="w-16 h-16 rounded-full bg-accent-neon/10 border-2 border-accent-neon/30 flex items-center justify-center mb-4">
          ...
          {current.hint && (
            <p className="text-xs text-accent-blue mt-3 bg-accent-blue/10 rounded-lg px-3 py-2">
              {current.hint}
            </p>
          )}
        </div>
